...
type LineKind = "cmd" | "out" | "sys" | "err" | "list" | "neofetch" | "stats" | "emu" | "matrix" | "me" | "whoami" | "snake" | "tetris" | "doom";
...
/**
 * The interactive terminal inside the laptop: Tab completion, ↑/↓ command history,
 * neofetch, live GitHub stats, themes and fonts — plus snake and a WebAssembly
 * DOOM build. Ported from Md. Danish's portfolio (github.com/danish9661/aboutme),
 * which was itself built from this one; the games and shell features are his work.
 */
export default function LaptopShowcase() {
  ...
  const run = useCallback(
    (raw: string) => {
      ...
      if (head === "ls" || cmd === "ll" || cmd === "dir") {
        ...
        if (targetId === "resume" || targetId === "pranav-shukla-resume.pdf" || targetId === "resume.pdf") {
          next.push({
            kind: "sys",
          ...
          if (proj) {
            ...
            next.push({
              kind: "err",
              text: `no such project: ${targetId || "—"} — try \`ls\` or \`open arbflow\``,
            });
          }
        }
      ...
      } else if (cmd === "visit site" || cmd === "site") {
        next.push({
          kind: "sys",
          text: `↗ Opening ${PORTFOLIO_CONFIG.socials.site} in a new tab...`,
        });
        if (typeof window !== "undefined") {
          window.open(PORTFOLIO_CONFIG.socials.site, "_blank", "noopener,noreferrer");
        }
        ...
        const headerLogs: Line[] = [
          ...
          { kind: "sys", text: "Hit:1 https://deb.pranavmshukla.in/pranavos stable InRelease" },
          { kind: "sys", text: "Get:2 https://deb.pranavmshukla.in/pranavos stable/main wasm-emulators [1,808 kB]" },
          { kind: "sys", text: "Get:3 https://deb.pranavmshukla.in/pranavos stable/main doom-wad-shareware [5,482 kB]" },
        ...
        if (cat === "lang" || cat === "languages") {
          next.push({ kind: "out", text: "• Languages: Python, C++, Rust, TypeScript / JavaScript, SQL, Dart, Java" });
        } else if (cat === "web" || cat === "backend") {
          next.push({ kind: "out", text: "• Web & Backend: Next.js, React, FastAPI, Flask, Flutter, REST APIs, WebAssembly, Tailwind CSS" });
        } else if (cat === "ml" || cat === "data") {
          next.push({ kind: "out", text: "• ML & Data: TensorFlow, Keras, Scikit-learn, Pandas, NumPy, SciPy · deep learning, time-series & signal processing, A/B testing" });
        } else if (cat === "infra" || cat === "tools") {
          next.push({ kind: "out", text: "• Infra & Tools: PostgreSQL, MySQL, SQLite, Docker, Git, Linux, Vercel, Looker Studio, Tableau, Power BI" });
        } else {
          next.push({
            kind: "out",
            text: "• Languages: Python, C++, Rust, TypeScript / JavaScript, SQL, Dart, Java\n• Web & Backend: Next.js, React, FastAPI, Flask, Flutter, REST APIs, WebAssembly, Tailwind CSS\n• ML & Data: TensorFlow, Keras, Scikit-learn, Pandas, NumPy, SciPy\n• Infra & Tools: PostgreSQL, MySQL, SQLite, Docker, Git, Linux, Vercel, Looker Studio\n(Tip: use `skills lang`, `skills web`, `skills ml`, or `skills infra` to filter)",
          });
        }
      ...
      } else if (cmd === "kleene" || cmd === "emu" || cmd === "boot") {
        next.push({ kind: "emu" });
      } else if (head === "whoami") {
        ...
        if (topic === "theme") {
          ...
          next.push({
            kind: "out",
            text: `commands: ls · open <project> · doom · snake · tetris · socials · email · skills [lang|web|ml|infra] · pranav · neofetch · stats · matrix · font <name|list> · theme <name|list|random> · history [clear] · uptime · uname -a · sudo <cmd> · date · kleene · resume · whoami${customCmdList ? ` · ${customCmdList}` : ""} · clear · home\n(Type \`help <command>\` for detailed usage)`,
          });
        }
      } else {
  ...
  const lineColor: Record<Exclude<LineKind, "cmd" | "list" | "neofetch" | "stats" | "emu" | "matrix" | "me" | "whoami" | "snake" | "tetris" | "doom">, string> =
    useMemo(
      () => ({
        out: "text-[#b9cbe0]",
                  ...
                  {view === "cli" && (
                    ...
                    >
                      ...
                      {history.map((line, i) => {
                        ...
                        if (line.kind === "me") {
                          return <MeOutput key={i} />;
                        }
                        if (line.kind === "whoami") {
                          return <WhoamiOutput key={i} />;
                        }
                        if (line.kind === "snake") {
                          return <SnakeGame key={i} />;
                        }
                        if (line.kind === "tetris") {
                          return <TetrisGame key={i} />;
                        }
                        if (line.kind === "doom") {
                        ...
                        return (
                          <div
                            key={i}
                            className={`mt-1.5 whitespace-pre-wrap ${lineColor[line.kind]}`}
                          >
                            {line.text}
                          </div>
                        );
...
{n.asciiArt}
      ...
      <div className="space-y-0.5 font-mono text-[12px] text-[#e7f0f9]">
        ...
        <p><span className="text-[#2563eb]">Shipped:</span> {n.shipped}</p>
        <p><span className="text-[#2563eb]">Languages:</span> {n.languages}</p>
        <p><span className="text-[#2563eb]">Focus:</span> {n.focus}</p>
...
function EmuOutput() {
  return (
    <div className="my-2 rounded-lg border border-[#28c840]/30 bg-[#0d1611] p-3 font-mono text-[11px] leading-relaxed text-[#28c840]">
      <p className="text-[#a2f0b0] font-bold">▶ [BOOT] Kleene core — instantiating the automata workspace...</p>
      <p>[WASM] 4-crate Rust workspace loaded (automata · conversions · equivalence)</p>
      <p>[CONV] regex → ε-NFA → DFA → minimal DFA → regex · every round traced</p>
      <p>[TEST] 10,000 randomized property cases per suite · differential vs the regex crate</p>
      <p className="text-white font-semibold">✓ Client-side. Inequivalent machines report the shortest string they disagree on.</p>
    </div>
  );
}
...
function MatrixOutput() {
  ...
  useEffect(() => {
    ...
    const chars = "01PRANAVWASM0101RUSTDFANFAｦｱｳｴｵｶｷｹｺｻｼｽｾｿﾀﾂﾃﾅﾆﾇﾈﾊﾋﾎﾏﾐﾑﾒﾓﾔﾕﾗﾘﾜ".split("");
    const fontSize = 12;
  ...
  return (
    <div className="my-2 overflow-hidden rounded-lg border border-[#28c840]/40 bg-black/90 p-2 font-mono text-[11px] select-none">
      <div className="flex items-center justify-between px-2 pb-1.5 text-[10px] text-[#28c840]/70">
        <span>▶ MATRIX DIGITAL RAIN [PRANAV_WASM_CORE]</span>
...
{PORTFOLIO_CONFIG.terminal.faceArt}
      </pre>
